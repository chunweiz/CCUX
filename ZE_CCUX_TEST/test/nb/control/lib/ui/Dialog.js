...
sap.ui.define(['jquery.sap.global', './library', 'sap/ui/core/Control', 'sap/ui/core/Popup'],
    function (jQuery, library, Control, Popup) {
        ...
        Dialog.prototype.onAfterRendering = function () {
            ...
            /*jslint nomen: true*/
            if (!Dialog._isSizeSet(this.getWidth()) && !Dialog._isSizeSet(this.getMaxWidth())) {/*jslint nomen: false*/
            ...
            if (!!sap.ui.Device.browser.internet_explorer && (sap.ui.Device.browser.version === 9 || sap.ui.Device.browser
                ...
                /*jslint nomen: true*/
                elm = $content[0];
                if (sap.ui.getCore().getConfiguration().getRTL() && !Dialog._isSizeSet(this.getWidth())) {/*jslint nomen: false*/
                    if (elm.ownerDocument && elm.ownerDocument.defaultView && elm.ownerDocument.defaultView.getComputedStyle) {
                        width = elm.ownerDocument.defaultView.getComputedStyle(elm).getPropertyValue("width");
                        if (width) {
                            fWidth  = parseFloat(width, 10);
                            if (fWidth % 1 === 0.5) {
          ...
          // Therefore we need to check which one of both cases is currently active.
            /*jslint nomen: true*/
            if (!Dialog._isSizeSet(this.getHeight()) && Dialog._isSizeSet(this.getMinHeight())) {/*jslint nomen: false*/
        ...
        */
        Dialog.prototype.onmousedown = function (oEvent) {
            ...
            if (jQuery.sap.containsOrEquals(this.getDomRef("hdr"), oSource)) {
                ...
                sWidth = oDomRef.offsetWidth + "px";
                oDomRef.style.width = sWidth;
                sHeight = oDomRef.offsetHeight + "px";
            ...
            oActElement = document.activeElement;
            if (oActElement && oActElement.id) {
                oCtrl = jQuery.sap.byId(oActElement.id).control(0);
                if (oCtrl) {
            ...
            outerDoc = Dialog._findSameDomainParentWinDoc();
            if (outerDoc) {
     ...
     */
        Dialog.prototype.handleMove = function (event) {
            ...
            if (this.sDragMode === "resize") {
                ...
                deltaX = event.screenX - this.startDragX;
                ...
                iLeft = this.originalRectangle.left + event.screenX - this.startDragX;
                iTop = this.originalRectangle.top + event.screenY - this.startDragY;
